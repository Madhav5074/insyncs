...
export default function JoinPage() {
  ...
  async function joinCircle() {
    ...
    try {
      // ✨ THE FIX: Check if they are using the exact same email/account
      if (circle.members?.includes(currentUser.uid)) {
        if (circle.members.length === 1) {
          // They are testing it on themselves! Throw an error.
          setError("You cannot join your own circle twice! To test this, please open an Incognito Window and log in with a completely different email address.");
          setIsJoining(false);
          return;
        } else {
          // If the circle is active and they are just returning, route them inside
          router.push(`/circle/${id}`);
          return;
        }
      }

      // Safety Check 2: Is it already full? (6 members max)
      if (circle.members?.length >= 6) {
        setError("This circle is already full!");
        setIsJoining(false);
        return;
      }

      // Add user to the main circle array
  ...
  const isFull = circle.members?.length >= 6;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black px-6 selection:bg-zinc-300 dark:selection:bg-zinc-700">
      <div className="w-full max-w-sm bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 shadow-2xl shadow-black/5 dark:shadow-white/5 animate-[fadeIn_0.5s_ease-out]">
        ...
        {isAlreadyMember && circle.members?.length > 1 ? (
           <button
          ...
          <button
            onClick={joinCircle}
            disabled={isJoining}
            className="w-full flex items-center justify-center gap-2 rounded-2xl bg-black py-4 text-white font-medium shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-200 hover:bg-zinc-800 hover:-translate-y-1 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed dark:bg-white dark:text-black"
          >
            {isJoining ? (
