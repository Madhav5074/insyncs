...
export default function GymTracker({ circle, me, circleId, todayKey }: any) {
  ...
  async function endWorkout() {
    ...
    const newStreak = (me.lastCheckin === getYesterday() ? (me.streak || 0) + 1 : 1);
      ...
      <div className="w-full p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs font-mono tracking-tight flex items-center gap-2">
              {hasLockedLocation ? (
                <>
                  <span className="text-green-600">✓</span> {me.lockedLocation.lat.toFixed(4)}° N
                </>
              ) : "⚠ Set Location Required"}
            </p>
          </div>
          <div className="px-4 py-3 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700">
             <span className="text-[10px] font-bold uppercase text-zinc-500">Locked 🔒</span>
          </div>
        </div>
        ...
        {hasLockedLocation && (
          <a 
            ...
            className="text-[10px] font-bold uppercase tracking-wider text-blue-500 flex items-center justify-center gap-1.5"
          >
            View on map 🗺️
          </a>
        )}
      ...
      <div className="pt-2">
        {currentState === 'completed' ? (
          ...
          <button onClick={verifyGymArrival} disabled={!hasLockedLocation || isLocating} className="w-full bg-black py-4 text-white text-lg font-bold rounded-2xl disabled:opacity-50 dark:bg-white dark:text-black">
            {isLocating ? "Verifying..." : "Verify GPS & Arrive"}
          </button>
        )}
      </div>
    </div>
